...
def execute_and_render_result(db: SQLDatabase, sql: str, using: Literal["dataframe", "tabulate", "json"]):
...
def _render_error(error: str, sql: str, using: str) -> Dict[str, Any]:
    ...
    if using == "dataframe":
        ...
        to_show["table"] = tabulate(pd.DataFrame([{"error": error}]), headers="keys", tablefmt="psql", showindex=False)
    elif using == "json":
...
def _render_data(result, sql: str, using: str) -> Dict[str, Any]:
    ...
    if using == "tabulate":
        to_show["table"] = tabulate(table, headers="keys", tablefmt="psql", showindex=False)
    elif using == "json":
