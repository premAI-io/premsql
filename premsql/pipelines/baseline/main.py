...
class BaseLineAgent(AgentBase):
    def __init__(
        ...
        self.followup_worker = BaseLineFollowupWorker(
            generator=specialized_model2
        )
    ...
    def run(
        ...
        if decision.route_to in ['query', 'analyse', 'plot']:
            ...
            if any(
                ...
                exit_output.followup_route_to_take = followup_output.alternative_route or "query" # This is the default route
                exit_output.error_from_followup_worker = followup_output.error_from_model
        else:
    ...
    def _handle_followup_route(self, question: str) -> ExitWorkerOutput:
        ...
        return ExitWorkerOutput(
            ...
            followup_route_to_take=followup_output.alternative_route or "query", # query should alaways be the default route
