...
class SessionComponent:
    ...
    def render_list_sessions(self):
        with st.sidebar:
            ...
            all_sessions = [session["session_name"] for session in all_sessions["data"]]
        ...
        with st.sidebar:
            ...
            with st.form(
                ...
                db_type = st.selectbox(
                    label="Select the database type", options=["sqlite", "postgres"]
                )
                config_path = st.file_uploader(
                    label="Upload custom config path", type=".py"
                )

                env_path = st.file_uploader(
                    label="Upload .env file if your agent needs", type=".env"
                )
            ...
            if button:
                ...
                if response["status"] == "success":
                    st.toast(
                        body=f"New session: {session_name} created. Please refresh",
                        icon="🥳",
                    )
                else:
