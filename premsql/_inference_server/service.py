...
def load_config_from_file(file_path):
    ...
    if hasattr(config_module, 'config') and hasattr(config_module, 'pipelines_mapping'):
        return (
            config_module.pipelines_mapping,
...
class AgentServer:
    def __init__(
        self,
        dsn_or_db_path: str,
        agent_name: str, config_path: Optional[str]=None,
        ...
        if config_path:
            self.pipeline_mapping, self.config_mapping = load_config_from_file(config_path)
        else:
            import premsql._inference_server.default_configs as server_config
            self.pipeline_mapping, self.config_mapping = (
                server_config.pipelines_mapping, server_config.config
            )
