...
class BaseGenerator(ABC):
    ...
    def generate_sql_file(self, response_list: list[str]) -> dict[int, str]:
        ...
        if  self.generator_config.data_output_path:
            ...
            json.dump(response_list, open(self.generator_config.data_output_path, "w"), indent=4)
            logger.info(f"all responses written to {self.generator_config.data_output_path}")
        
    
    def generate_and_save_results(
            self, data: list[dict], postprocess: Optional[bool]=False,
