...
def package_sqls(
    sql_path, db_root_path, num_rows: Optional[int] = None, mode="gpt", data_mode="dev"
):
    clean_sqls = []
    db_path_list = []
    sql_data = json.load(open(sql_path + "predict_" + data_mode + ".json", "r"))
    num_rows = len(sql_data) if num_rows is None else num_rows
    ...
    if mode == "gpt":
        ...
        sqls = open(sql_path + data_mode + ".sql")  #'_gold.sql')#'_gold.sql')
        sql_txt = sqls.readlines()
        # sql_txt = [sql.split('\t')[0] for sql in sql_txt]
        for idx, sql_str in enumerate(sql_txt[:num_rows]):
...
def compute_acc_by_diff(exec_results, diff_json_path, num_rows: Optional[int] = None):
...
if __name__ == "__main__":
    ...
    simple_acc, moderate_acc, challenging_acc, acc, count_lists = compute_acc_by_diff(
        exec_result, args.diff_json_path, args.num_rows
    )
