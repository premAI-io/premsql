        ...
        if filter_used is not None:
            ...
            if filter_key == "difficulty":
                ...
                res = {
                    ...
                    **dict(zip(list(set(["simple", "moderate", "challenging"]) - set([filter_value])), [(0,0), (0,0)]))
                }
                return res
        else:
            results = {
            ...
            return {
                "simple": (self.compute_metric(results["simple"]) if len(results["simple"]) > 0 else 0, len(results["simple"])),
                "moderate": (self.compute_metric(results["moderate"]) if len(results["moderate"]) > 0 else 0, len(results["moderate"])),
                "challenging": (self.compute_metric(results["challenging"]) if len(results["challenging"]) > 0 else 0, len(results["challenging"])),
