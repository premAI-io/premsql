...
class DataInstance:
    ...
    def apply_prompt(self, apply_knowledge: Optional[bool] = False, header_prompt: Optional[str]=None) -> str:
...
class BirdBenchEvalDataset:
    ...
    def data(self) -> dict:
        return self.download(
            download_path=self.config.db_root_path,
            force=False
        )

    def process_and_filter(self, filter_by: Optional[tuple]=None, num_rows: Optional[int]=None) -> dict:
        """
        ...
        if filter_by is not None:
            ...
            if filter_key == "difficulty":
                assert filter_value in ["simple", "moderate", "challenging"], ValueError(
        ...
        for content in data:
            curr_db_path = (
                self.config.db_root_path + self.config.databases_folder_name + content["db_id"] + "/" + content["db_id"] + ".sqlite"
            )
    ...
    def download(self, download_path: Optional[str] = "./data/eval", force: Optional[bool]=False):
        ...
        def perform_download_and_extraction():
            ...
            logger.info("=> Starting to download the dataset for evaluation [BirdBench devset].")
        ...
        if os.path.isdir(download_path):
            if force:
                ...
                logger.info(f"{download_path} is not empty. Use force=True to re-download and overwrite the contents.")
        else:
            perform_download_and_extraction()
